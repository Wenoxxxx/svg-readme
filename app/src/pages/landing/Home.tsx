...
export default function Home() {
  ...
  const [tagline, setTagline] = useState("BSIT student · freelance designer · builder");
  ...
  <text class="${uid}-handle" x="${(w * 0.065 + 18).toFixed(1)}" y="${(h * 0.30).toFixed(1)}" font-size="${Math.round(h * 0.16)}" font-weight="600" fill="${ink}">${esc(handle)}</text>
    ...
    <div className="bg-white min-h-screen text-zinc-900 font-sans">
        ...
        <div className="max-w-[1180px] mx-auto px-6">
          <div className="max-w-[640px] mb-9">
            ...
            <h1 className="font-['Poppins'] font-semibold text-3xl md:text-5xl leading-[1.08] tracking-tight text-zinc-900">
              An animated banner for your README, built <em className="text-[#1b5def] not-italic">in the browser</em>, not in Figma.
            </h1>
            <p className="text-[16.5px] text-zinc-500 mt-4 max-w-[52ch] leading-relaxed">
              Type your handle and tagline, pick an accent and a motion style, and export a real SVG with the keyframes baked in — no rendering pipeline required to try it.
            </p>
          </div>

          {/* Interactive Studio */}
          <div className="grid grid-cols-1 md:grid-cols-[340px_1fr] border border-zinc-900" id="studio">
            {/* Control Panel */}
            <div className="p-[22px] border-b md:border-b-0 md:border-r border-zinc-900 bg-white flex flex-col gap-6">
              <div>
                <div className="font-mono text-[11px] uppercase tracking-[0.08em] text-zinc-400 mb-3.5">01 — Content</div>
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[12.5px] text-zinc-500 font-mono" htmlFor="handle-in">Handle / name</label>
                    <input
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[12.5px] text-zinc-500 font-mono" htmlFor="tagline-in">Tagline</label>
                    <input
                      className="w-full border border-zinc-200 bg-white px-2.5 py-2 font-sans text-sm text-zinc-900 rounded-none focus:outline-none focus:ring-2 focus:ring-[#1b5def] focus:border-[#1b5def]"
                      id="tagline-in"
                      type="text"
              ...
              <div>
                <div className="font-mono text-[11px] uppercase tracking-[0.08em] text-zinc-400 mb-3.5">02 — Accent</div>
                <div className="flex gap-2 flex-wrap">
                  {swatches.map((s) => (
                    <button
                      key={s.value}
                      className={`w-7 h-7 border border-zinc-900 cursor-pointer relative shrink-0 ${
                        color === s.value ? "after:content-[''] after:absolute after:-inset-1 after:border after:border-zinc-900" : ""
                      }`}
              ...
              <div>
                <div className="font-mono text-[11px] uppercase tracking-[0.08em] text-zinc-400 mb-3.5">03 — Motion</div>
                <div className="flex border border-zinc-200">
                  <button
                    className={`flex-1 py-2 text-xs font-mono border-r border-zinc-200 last:border-r-0 cursor-pointer ${
                      motion === "fade" ? "bg-zinc-900 text-white" : "bg-white text-zinc-500"
                    }`}
                  ...
                  <button
                    className={`flex-1 py-2 text-xs font-mono border-r border-zinc-200 last:border-r-0 cursor-pointer ${
                      motion === "sweep" ? "bg-zinc-900 text-white" : "bg-white text-zinc-500"
                    }`}
              ...
              <div>
                <div className="font-mono text-[11px] uppercase tracking-[0.08em] text-zinc-400 mb-3.5">04 — Size &amp; theme</div>
                <div className="flex flex-col gap-4">
                  <select
                  ...
                  <div className="flex border border-zinc-200">
                    <button
                      className={`flex-1 py-2 text-xs font-mono border-r border-zinc-200 last:border-r-0 cursor-pointer ${
                        theme === "light" ? "bg-zinc-900 text-white" : "bg-white text-zinc-500"
                      }`}
                    ...
                    <button
                      className={`flex-1 py-2 text-xs font-mono border-r border-zinc-200 last:border-r-0 cursor-pointer ${
                        theme === "dark" ? "bg-zinc-900 text-white" : "bg-white text-zinc-500"
                      }`}
            ...
            <div className="bg-zinc-950 p-[22px] flex flex-col gap-4 min-w-0 relative shadow-[inset_0_0_100px_rgba(0,0,0,0.5)] overflow-hidden border-t md:border-t-0 border-zinc-900">
              ...
              <div className="font-mono text-[11px] uppercase tracking-[0.08em] text-zinc-500 z-10">Live preview</div>
              ...
              <div className="flex gap-2.5 items-center flex-wrap z-10">
                <button className="font-mono text-[13px] px-4 py-2.5 bg-[#1b5def] text-white border-0 hover:bg-[#154bc4] cursor-pointer whitespace-nowrap" onClick={handleDownload}>
                  Download SVG
                </button>
                <button className="font-mono text-[13px] px-4 py-2.5 bg-zinc-900 text-zinc-100 border border-white/10 hover:bg-zinc-800 cursor-pointer whitespace-nowrap" onClick={handleCopyMarkdown}>
                  Copy README markdown
                </button>
                <span className="font-mono text-xs text-[#1b5def] h-3.5 transition-opacity duration-300">{toast}</span>
              </div>
        ...
        <div className="max-w-[1180px] mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-zinc-200 border border-zinc-200">
            <div className="bg-white p-[26px]">
              ...
              <h3 className="font-['Poppins'] font-semibold text-[17px] text-zinc-900 mb-2">Real keyframes, not frames</h3>
              <p className="text-sm text-zinc-500 leading-relaxed">
                The animation is CSS inside the SVG's own &lt;style&gt; block — it plays natively wherever the SVG renders, with no frame count and no file-size penalty for smoothness.
              </p>
            </div>
            <div className="bg-white p-[26px]">
              <div className="font-mono text-xs text-[#1b5def] mb-3.5">02</div>
              <h3 className="font-['Poppins'] font-semibold text-[17px] text-zinc-900 mb-2">What you see is what ships</h3>
              <p className="text-sm text-zinc-500 leading-relaxed">
                The preview on this page is the exact markup you download. No separate render step, no "close enough" gap between the editor and the export.
              </p>
            </div>
            <div className="bg-white p-[26px]">
              <div className="font-mono text-xs text-[#1b5def] mb-3.5">03</div>
              <h3 className="font-['Poppins'] font-semibold text-[17px] text-zinc-900 mb-2">Your brand, not a template</h3>
              <p className="text-sm text-zinc-500 leading-relaxed">
                Accent, size, and motion are parameters, not presets baked into someone else's generator — set them once to match the rest of your profile.
              </p>
            </div>
          </div>
        </div>
            ...
            <div>
              ...
              <ol className="list-none p-0 m-0 flex flex-col gap-6">
                <li className="relative pl-[38px]">
                  <span className="absolute left-0 top-0 font-mono text-xs w-6 h-6 border border-zinc-900 flex items-center justify-center bg-white text-zinc-950">1</span>
                  <h4 className="font-['Poppins'] font-semibold text-[15px] text-zinc-900 mb-1">Layout via React + Satori</h4>
                  <p className="text-[13.5px] text-zinc-500 leading-relaxed">Content and layout are described as React, then flattened to SVG server-side — the same approach used for OG-image generation.</p>
                </li>
                <li className="relative pl-[38px]">
                  <span className="absolute left-0 top-0 font-mono text-xs w-6 h-6 border border-zinc-900 flex items-center justify-center bg-white text-zinc-950">2</span>
                  <h4 className="font-['Poppins'] font-semibold text-[15px] text-zinc-900 mb-1">Keyframes injected after render</h4>
                  <p className="text-[13.5px] text-zinc-500 leading-relaxed">Satori's output has no animation support, so the motion CSS is hand-written and injected into the SVG's &lt;style&gt; as a post-process step.</p>
                </li>
                <li className="relative pl-[38px]">
                  <span className="absolute left-0 top-0 font-mono text-xs w-6 h-6 border border-zinc-900 flex items-center justify-center bg-white text-zinc-950">3</span>
                  <h4 className="font-['Poppins'] font-semibold text-[15px] text-zinc-900 mb-1">Export, then commit</h4>
                  <p className="text-[13.5px] text-zinc-500 leading-relaxed">Download the SVG, drop it in your repo, and reference it with a standard markdown image tag — GitHub renders the animation as-is.</p>
                </li>
              </ol>
            </div>
            <div className="bg-[#0f1117] text-[#e7e9ee] p-5 font-mono text-[12.5px] leading-relaxed border border-zinc-900 overflow-x-auto">
              <span className="text-[#7d8296]"># README.md</span><br />
              <span className="text-[#9fe08a]">![banner](./banner.svg)</span><br /><br />
              <span className="text-[#7d8296]">&lt;!-- that's it — no build step, --&gt;</span><br />
              <span className="text-[#7d8296]">&lt;!-- no external image host, --&gt;</span><br />
              <span className="text-[#7d8296]">&lt;!-- no action to keep running --&gt;</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="py-7 bg-white">
        <div className="max-w-[1180px] mx-auto px-6 flex justify-between items-center flex-wrap gap-3">
          <span className="font-mono text-[12.5px] text-zinc-400">svg-readme — built by Owen Jerusalem</span>
          <a className="text-[12.5px] text-zinc-400 hover:text-zinc-900 transition-colors no-underline" href="https://github.com/Wenoxxxx" target="_blank" rel="noopener noreferrer">
            github.com/Wenoxxxx
          </a>
        </div>
      </footer>
