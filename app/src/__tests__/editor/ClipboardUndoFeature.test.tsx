...
describe("Clipboard and Undo Features — Unit Tests", () => {
  ...
  describe("Copy Feature (Ctrl+C)", () => {
    ...
    it("should copy multiple selected layers to clipboard", () => {
      ...
      expect(clipboardState.layers.some(l => l.id === "layer-1")).toBe(true);
      expect(clipboardState.layers.some(l => l.id === "layer-2")).toBe(true);
    });
    ...
    it("should copy layer properties along with layers", () => {
      ...
      const clipboardState = setClipboard.mock.calls[0][0];
      expect(clipboardState.elementProperties["layer-1"]).toEqual(mockElementProperties["layer-1"]);
    });
  });
  ...
  describe("Paste Feature (Ctrl+V)", () => {
    ...
    it("should do nothing when clipboard is empty", () => {
      const { setLayers, handlePaste } = createHandlePaste(
        null,
        mockLayers,
        mockElementProperties
      );
    ...
    it("should paste single layer from clipboard", () => {
      const clipboard: ClipboardState = {
        layers: [{ id: "copied-1", name: "Copied Layer", type: "text", locked: false, visible: true }],
        elementProperties: {
          "copied-1": {
            type: "text",
            x: 100,
      ...
      const { setLayers, setElementProperties, setSelectedLayerIds, setSelectedLayerId, handlePaste } = 
    ...
    it("should paste multiple layers from clipboard", () => {
      const clipboard: ClipboardState = {
        layers: [
          { id: "copied-1", name: "Copied Layer 1", type: "text", locked: false, visible: true },
          { id: "copied-2", name: "Copied Layer 2", type: "text", locked: false, visible: true }
        ],
        elementProperties: {
      ...
      const { setLayers, handlePaste } = createHandlePaste(clipboard, mockLayers, mockElementProperties);
    ...
    it("should generate new IDs for pasted layers", () => {
      const clipboard: ClipboardState = {
        layers: [{ id: "original-id", name: "Original", type: "text", locked: false, visible: true }],
        elementProperties: {
    ...
    it("should offset position of pasted layers", () => {
      const clipboard: ClipboardState = {
        layers: [{ id: "copied-1", name: "Copied", type: "text", locked: false, visible: true }],
        elementProperties: {
          "copied-1": {
            type: "text",
            x: 100,
      ...
      const { setElementProperties, handlePaste } = createHandlePaste(clipboard, mockLayers, mockElementProperties);
      ...
      if (typeof setElementPropsCall === 'function') {
        ...
        const pastedProps = Object.values(result).find(p => p.content === "Copied Content");
        if (pastedProps) {
    ...
    it("should select pasted layers after pasting", () => {
      ...
      const { setSelectedLayerIds, setSelectedLayerId, handlePaste } = createHandlePaste(
        clipboard, mockLayers, mockElementProperties
      );
  ...
  describe("Undo Feature (Ctrl+Z)", () => {
    ...
    it("should undo layer deletion", () => {
      ...
      const { setLayers, setElementProperties, setSelectedLayerIds, handleUndo } = 
        createHandleUndo(
